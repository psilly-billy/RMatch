...
template_path = r"tests\ATS_template.docx"
json_path = r"tests\profile.json"
...
def replace_text_in_element(element, replacements, formatting_map):
    for key, value in replacements.items():
        if key in element.text:
            print(f"Replacing {key} with {value} in element text: {element.text}")
            if key == '*Linkedin' or key == '*Github':
                ...
                for run in element.runs:
                    if key in run.text:
                        print(f"Updating run text: {run.text}")
                        run.text = run.text.replace(key, value)
                        apply_formatting(run, formatting_map.get(key, extract_formatting(run)))
                # If the key is found, replace the text directly in the element to avoid duplication
...
def replace_name(doc, name, formatting_map):
    for para in doc.paragraphs:
        for run in para.runs:
            if '*Name' in run.text:
                print(f"Replacing *Name with {name} in run text: {run.text}")
...
def check_and_replace(doc, replacements, formatting_map):
    for paragraph in doc.paragraphs:
        print(f"Paragraph text before replacement: {paragraph.text.strip()}")
        replace_text_in_element(paragraph, replacements, formatting_map)
        print(f"Paragraph text after replacement: {paragraph.text.strip()}")

    for table in doc.tables:
        for row in table.rows:
            for cell in row.cells:
                for paragraph in cell.paragraphs:
                    print(f"Cell paragraph text before replacement: {paragraph.text.strip()}")
                    replace_text_in_element(paragraph, replacements, formatting_map)
                    print(f"Cell paragraph text after replacement: {paragraph.text.strip()}")
...
# Save the modified document in the 'tests' folder with a new name
output_path = os.path.join("tests", "Populated_ATS_template_v2.docx")
try:
